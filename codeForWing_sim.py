...
step_folder_name = '/Step-'
slice_folder_name = '/Xfoil_output/Sections'
res_folder_name = '/ABQ-Results'
odbfile_path = start_dir+'/'+jobname                                            # path of the odb-file which will be created in the working directory
...
anzRibs = 6
t_stringer = float(paraRead_wing.t_stringer)
element_width = width/N
...
P_i = P_i - design.cutWingRoot
Q_i = Q_i - design.cutWingRoot
P_j = P_j + (design.mod8_DN[1] - design.cutDown)
Q_j = Q_j + (design.mod8_DN[1] - design.cutDown)

for q_i in Q_i:
    if not q_i in [Q_i[0], Q_i[-1]]:
        datumPlane_partitionSkin = WingModel.parts['Wing'].DatumPlaneByPrincipalPlane(offset=q_i, principalPlane=XYPLANE)
        WingModel.parts['Wing'].PartitionFaceByDatumPlane(datumPlane=WingModel.parts['Wing'].datums[datumPlane_partitionSkin.id], faces=WingModel.parts['Wing'].sets['box_set'].faces)
...
for i in range(Anz_Unterteilung):   # REIHENFOLGE : OBEN HINTEN ; OBEN VORNE ; UNTEN VORNE ; UNTEN HINTEN
    WingModel.rootAssembly.Set(name='Point-OL-'+str(i),  vertices=WingModel.rootAssembly.instances['Wing-final-1'].vertices.getByBoundingSphere((points_nose[0][0], points_nose[0][1], y_C[N+i]), 1E-03))
    WingModel.rootAssembly.Set(name='Point-OR-'+str(i),  vertices=WingModel.rootAssembly.instances['Wing-final-1'].vertices.getByBoundingSphere((points_box_up[0][0], points_box_up[0][1], y_C[N+i]), 1E-03))
    WingModel.rootAssembly.Set(name='Point-UR-'+str(i),  vertices=WingModel.rootAssembly.instances['Wing-final-1'].vertices.getByBoundingSphere((points_box_low[-1][0], points_box_low[-1][1], y_C[N+i]), 1E-03))
    WingModel.rootAssembly.Set(name='Point-UL-'+str(i),  vertices=WingModel.rootAssembly.instances['Wing-final-1'].vertices.getByBoundingSphere((points_nose[-1][0], points_nose[-1][1], y_C[N+i]), 1E-03))
##
...
for y in xrange(1,anzRibs+1):
    WingModel.rootAssembly.Set(name='LEAD-'+str(int(anzRibs)-y),  vertices=WingModel.rootAssembly.instances['Wing-final-1'].vertices.getByBoundingSphere((0, 0, width-(y-1)*anzDivision*width/Anz_Unterteilung), 1E-03))     
    WingModel.rootAssembly.Set(name='REAR-'+str(int(anzRibs)-y),  vertices=WingModel.rootAssembly.instances['Wing-final-1'].vertices.getByBoundingSphere((points_trail4_up[0][0], 0, width-(y-1)*anzDivision*width/Anz_Unterteilung), 1E-03))
...
#WingModel.rootAssembly.Set(name='point_SC',  vertices=WingModel.rootAssembly.instances['Wing-final-1'].vertices.getByBoundingBox(x_SC-1E-4, 0.2*0.12*c, width-1E-4, x_SC+1E-4, 0.12*c, width+1E-4))

WingModel.rootAssembly.regenerate()

# Output front for averaging stresses



faces_front_root_UpperLower=WingModel.rootAssembly.instances['Wing-final-1'].faces.findAt(
                                                                ( ( points_nose[1][0], points_nose[1][1], 0.001 ) ,),
                                                                ( ( points_nose[-2][0], points_nose[-2][1], 0.0001 ) ,),)

faces_front_root_Upper=WingModel.rootAssembly.instances['Wing-final-1'].faces.findAt(
                                                                ( ( points_nose[1][0], points_nose[1][1], 0.001 ) ,),)

faces_rear_root_Upper=WingModel.rootAssembly.instances['Wing-final-1'].faces.findAt( ( ( points_trail1_up[1][0],points_trail1_up[1][1],0.001 ) ,),)

faces_rear_root_UpperLower=WingModel.rootAssembly.instances['Wing-final-1'].faces.findAt( ( ( points_trail1_up[1][0],points_trail1_up[1][1],0.001 ) ,),
                                                                   ( ( points_trail1_low[1][0], points_trail1_low[1][1], 0.001 ) ,))




#### faces
##setfrontrootUpLow=WingModel.rootAssembly.Set(faces=faces_front_root_UpperLower , name='AUSLESEN_F_UL')
##setfrontrootUp=WingModel.rootAssembly.Set(faces=faces_front_root_Upper , name='AUSLESEN_F_U')
##setrearrootUpLow=WingModel.rootAssembly.Set(faces=faces_rear_root_UpperLower , name='AUSLESEN_R_UL')
##setrearrootUp=WingModel.rootAssembly.Set(faces=faces_rear_root_Upper , name='AUSLESEN_R_U')
###Surfaces
##WingModel.rootAssembly.Surface(name='AUSLESEN_SURF_F_UL', side1Faces=faces_front_root_UpperLower)
##WingModel.rootAssembly.Surface(name='AUSLESEN_SURF_F_U', side1Faces=faces_front_root_Upper)
##WingModel.rootAssembly.Surface(name='AUSLESEN_SURF_R_UL', side1Faces=faces_rear_root_UpperLower)
##WingModel.rootAssembly.Surface(name='AUSLESEN_SURF_R_U', side1Faces=faces_rear_root_Upper)
#### Elements
##WingModel.rootAssembly.Set(elements=setfrontrootUpLow.elements, name='AUSLESEN_F_UL_ELEM')
##WingModel.rootAssembly.Set(elements=setfrontrootUp.elements, name='AUSLESEN_F_U_ELEM')
##WingModel.rootAssembly.Set(elements=setrearrootUpLow.elements, name='AUSLESEN_R_UL_ELEM')
##WingModel.rootAssembly.Set(elements=setrearrootUp.elements, name='AUSLESEN_R_U_ELEM')
#### Nodes
##WingModel.rootAssembly.Set(nodes=setfrontrootUpLow.nodes, name='AUSLESEN_F_UL_NODES')
##WingModel.rootAssembly.Set(nodes=setfrontrootUp.nodes, name='AUSLESEN_F_U_NODES')
##WingModel.rootAssembly.Set(nodes=setrearrootUpLow.nodes, name='AUSLESEN_R_UL_NODES')
##WingModel.rootAssembly.Set(nodes=setrearrootUp.nodes, name='AUSLESEN_R_U_NODES')


# SetsByBoundingBox

set_All_Elements=WingModel.rootAssembly.Set(elements=WingModel.rootAssembly.instances['Wing-final-1'].elements.getByBoundingBox(-10,-10,-10,10,10,10), name='A_ALL_ELEMENTS')

set_All_Elements_NoTip=WingModel.rootAssembly.Set(elements=WingModel.rootAssembly.instances['Wing-final-1'].elements.getByBoundingBox(-10,-10,-10,10,10,1.4), name='A_ALL_ELEMENTS_NO_TIP')

set_Front_ObenBB=WingModel.rootAssembly.Set(elements=WingModel.rootAssembly.instances['Wing-final-1'].elements.getByBoundingBox(145e-3,20e-3,1.49,155e-3,40e-3,1.53), name='A_FRONT_CORNER')

WingModel.rootAssembly.SetByBoolean(name='A_ALL_ELEMENTS_NO_TIP_CORNER', 
    operation=DIFFERENCE, sets=(
    WingModel.rootAssembly.sets['A_ALL_ELEMENTS'], 
    WingModel.rootAssembly.sets['A_FRONT_CORNER']))

set_Front_ObenBB=WingModel.rootAssembly.Set(elements=WingModel.rootAssembly.instances['Wing-final-1'].elements.getByBoundingBox(0.08,2.7E-02,0.0,0.1001,1,0.021), name='A_FRONT_OBEN_ELEMENTS')
set_Front_UntenBB=WingModel.rootAssembly.Set(elements=WingModel.rootAssembly.instances['Wing-final-1'].elements.getByBoundingBox(0.08,-1,0.0,0.1001,-2.7E-02,0.021), name='A_FRONT_UNTEN_ELEMENTS')
setRear_ObenBB=WingModel.rootAssembly.Set(elements=WingModel.rootAssembly.instances['Wing-final-1'].elements.getByBoundingBox(0.1499,2.7E-02,0.0,0.17,1,0.021), name='A_REAR_OBEN_ELEMENTS')
setRear_UntenBB=WingModel.rootAssembly.Set(elements=WingModel.rootAssembly.instances['Wing-final-1'].elements.getByBoundingBox(0.1499,-1,0.0,0.17,-2.7E-02,0.021), name='A_REAR_UNTEN_ELEMENTS')

WingModel.rootAssembly.SetByBoolean(name='ALL_ELEMENTS_WITHOUT_WARPING_EDGES', 
    operation=DIFFERENCE, sets=(
    WingModel.rootAssembly.sets['A_ALL_ELEMENTS'], 
    WingModel.rootAssembly.sets['A_FRONT_OBEN_ELEMENTS'], 
    WingModel.rootAssembly.sets['A_FRONT_UNTEN_ELEMENTS'], 
    WingModel.rootAssembly.sets['A_REAR_OBEN_ELEMENTS'], 
    WingModel.rootAssembly.sets['A_FRONT_CORNER'], 
    WingModel.rootAssembly.sets['A_REAR_UNTEN_ELEMENTS']))
###############################

#####
#Boundary conditions
instanceToApplyMeshBCsLoads = model.rootAssembly.instances['Wing-final-1']

design = defineBCs(model, design, instanceToApplyMeshBCsLoads, load, load.typeBC)

design = defineBCs(model, design, instanceToApplyMeshBCsLoads, load, 'couplingAtLatticeNodes')

if (design.cutGap_y != 0.0 and load.additionalBC != 'none'):# or load.conditionNodesInnerLattice == 'tyre':

    design = defineBCs(model, design, instanceToApplyMeshBCsLoads, load, load.additionalBC)